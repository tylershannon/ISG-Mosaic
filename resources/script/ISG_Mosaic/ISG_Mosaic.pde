...
//INCREMENT COUNTERS FOR NEXT IMAGE
    ...
    portrait_1 = loadImage("resources/images/GardnerPortrait-16by9.jpg");
    pixelateImage(insta_size, -3, 0, 50);
    delay(100);
}
...
void accessAPI(String api_address) {
  ...
  for (int i=0; i<post.size(); i++){
    try {
      ...
      img_get.save("output/result"+(109+i)+".jpg");
    }
    catch(Exception e){
      ...
      img_proxy.save("output/result"+(109+i)+".jpg");
    }
  }
  for (int i=0; i < post.size(); i++) {
...
void pixelateImage(int pxSize, int x_shift, int y_shift, int alpha) {
  ...
  int px_x_shift = x_shift*pxSize;
  int px_y_shift = y_shift*pxH;
  
  noStroke();
  pushMatrix();
  translate(px_x_shift, px_y_shift);
  for (int x=0; x<portrait_1.width; x+=pxSize) {
    for (int y=0; y<height; y+=pxH) {
      fill(portrait_1.get(x, y), alpha);
      rect(x, y, pxSize, pxH);
    }
  }
  popMatrix();
